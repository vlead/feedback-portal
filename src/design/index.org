...
* Dynamic generation of =feedback form=
  ...
  the [[./index.org::#api][=GET=]] response.  The response contains all the relevant data to build the
  form.  It will hold the questions, the type like if the question is a radio
  button or a check box etc. and the values for each option.
