#+TITLE: Integration of Feedback with Elasticsearch
#+AUTHOR: VLEAD
#+DATE: [2016-07-26 Tue]
#+SETUPFILE: ../../org-templates/level-2.org
#+TAGS: boilerplate(b)
#+EXCLUDE_TAGS: boilerplate
#+OPTIONS: ^:nil

* Introduction
  This document explains the code snippet which is integrated to =elasticsearch= service 
* Implementation
** Configuration
  Configuration variable of both =feedback= and =elasticsearch= micro services.
#+BEGIN_SRC python :tangle config.py
FEEDBACK_URL = "http://localhost:5000"
KEY = "place-key-here"
#+END_SRC
** Dump regular feedback data to elastic search database.
   Following code computes the date of previous day (based on server
   time) and invoke http request to =feedback= service for computed
   date to get the feedback dump and pushes them to =elasticsearch=
   database. This will happen on every midnight (cron job basis)
   
#+BEGIN_SRC python :tangle feedback_elastic_search.py
import os
import requests
import json
from config import *
from datetime import datetime, timedelta, date
from dateutil.rrule import rrule, DAILY

yestr_day = datetime.now() - timedelta(days=1)
yestr_day_date = yestr_day.strftime('%d-%m-%Y')

FEEDBACK_DUMP_URL = "%s/feedback_dump?date=%s&key=%s" % (FEEDBACK_URL, yestr_day_date, KEY)
ELASTIC_DUMP_URL = "%s/dump_feedback_to_elastic_db" % ("http://feedback-kebana.base4.vlabs.ac.in")

def get_feedback_dump(API_URL):
    try:
	resp = requests.get(API_URL)
	feedback_dump = None
	if resp.status_code == 200:
	    feedback_dump = resp.json()
	    return (resp.status_code, feedback_dump)
	else:
	    return (resp.status_code, feedback_dump)
    except Exception as e:
	return 

def post_feedback_to_elastic_db(feedbacks):
    try:
        headers = {'content-type': 'application/json'}
	resp = requests.post(ELASTIC_DUMP_URL, data=json.dumps(feedbacks), headers=headers)
	if not resp.status_code == 200:
	    return False
	else:
	    return True
    except Exception as e:
	print str(e)
	return False

(status_code, feedback_dump) = get_feedback_dump(FEEDBACK_DUMP_URL)

if(status_code == 200):
    data_dict = {
	"feedbacks" : feedback_dump,
	"key" : KEY
    }
    if(post_feedback_to_elastic_db(data_dict)):
	print "Feedbacks : %s is added" % (feedback_dump)
    else:
	print status_code
else:
    print "Error in getting feedback dump"

#+END_SRC
** Dump historic feedback into elastic search database
   Follwing code dumps historic feedback data (list of feedbacks in
   json format) into elastic search database.
#+BEGIN_SRC python :tangle historic_feedback.py
from config import *
from datetime import datetime, timedelta, date
from dateutil.rrule import rrule, DAILY
from feedback_elastic_search import *

yestr_day = datetime.now() - timedelta(days=1)
yestr_day_date = yestr_day.strftime('%d-%m-%Y')

def dump_historic_feedback_data(start_date, end_date):

    for dt in rrule(DAILY, dtstart=start_date, until=end_date):
	date = dt.strftime("%d-%m-%Y")
	FEEDBACK_DUMP_URL = "%s/feedback_dump?date=%s&key=%s" % (FEEDBACK_URL, date, KEY)
        try:               
            (status_code, feedback_dump) = get_feedback_dump(FEEDBACK_DUMP_URL)                                            
        except Exception as e:                               
            print "No feedbacks found on date : %s" % (date) 

	if(status_code == 200):
	    data_dict = {
		"feedbacks" : feedback_dump,
		"key" : KEY
	    }
	    if(post_feedback_to_elastic_db(data_dict)):
		print "Feedbacks : %s is added" % (feedback_dump)
	    else:
		print status_code
	else:
	    print "Error in getting feedback dump on date :%s " % (date)

start_date = date(2015, 01, 01)
end_date = date(2017, 03, 30)
dump_historic_feedback_data(start_date, end_date)

#+END_SRC
   
