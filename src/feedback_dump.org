...
ELASTIC_DUMP_URL = "%s/dump_feedback_to_elk" % (FEEDBACK_URL)
...
def post_feedback_to_elastic_db(feedbacks):
    try:
	resp = requests.post(ELASTIC_DUMP_URL, data=json.dumps(feedbacks))
