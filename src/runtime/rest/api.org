...
#+BEGIN_SRC python
def post_data_to_elastic_search(index_name, doc_name, server_name, record):
    try:
        current_app.logger.debug("index_name: %s, doc_name: %s, server_name: %s, record: %s" % (index_name, doc_name, server_name, record))
        ELASTIC_SEARCH_URL = "%s/%s/%s" % (server_name, index_name, doc_name)
...
#+BEGIN_SRC python
@api.route('/dump_college_details_to_elk', methods=['GET', 'POST'])
def dump_feedback_to_elk():
	...
	config = data_dict
	key = data_dict['key']
	if(post_data_to_elastic_search(index_name, doc_name, server_name, feedback)):
	    current_app.logger.debug("Added college_details : %s " % (config))

        else:
            current_app.logger.error("failed to add config: %s " % (config))
            abort(500)
...
def dump_feedback_to_elk():
	...
	key = data_dict['key']
        college_name = requests.get("url/college_details?id=mac_id")
...
def dump_usage_to_elk():
	...
	key = data_dict['key']
	for usage in usages:
	    if(post_data_to_elastic_search(index_name, doc_name, server_name, usage)):
