...
def post_data_to_elastic_search(index_name, doc_name, record):                                                                                          
    try:                                                                                                                                                
        current_app.logger.debug("index_name: %s, doc_name: %s, record: %s" % (index_name, doc_name, record))                                           
        ELASTIC_SEARCH_URL = "%s/%s/%s" % ("http://10.100.0.15:9200", index_name, doc_name)                                                             
        current_app.logger.debug("ELASTIC_SEARCH_URL : %s" %(ELASTIC_SEARCH_URL))                                                                       
        resp = requests.post(ELASTIC_SEARCH_URL, data=json.dumps(record))                                                                               
...
** Dump College Details to ELK
   REST API to add college details to ELK

#+NAME: add_college_details_to_elk
#+BEGIN_SRC python
@api.route('/dump_cc_details_to_elk', methods=['GET', 'POST'])
def dump_cc_details_to_elk():
    if request.method == 'GET':
	return True
    if request.method == 'POST':
	try:
	    #ELASTIC_SEARCH_URL = "http://10.100.0.15:9200/college_cloud/details"
	    data_json = json.dumps(request.get_json())
	    data_dict = yaml.safe_load(data_json)
            if 'college_details' not in data_dict:
                abort(400, 'college_details attribute is not passed')
	    ...
	    mac_id = cc_details['mac_id']            
	    ELASTIC_SEARCH_GET_URL = "%s/id=%s" %(ELASTIC_SEARCH_URL, str(mac_id))
	    current_app.logger.debug("ELASTIC SEARCH GET URL : %s " % (ELASTIC_SEARCH_GET_URL))
	    college_data = requests.get(ELASTIC_SEARCH_GET_URL)
	    if college_data.json()['found'] == False:
		ELASTIC_SEARCH_POST_URL = "%s/id=%s" % (ELASTIC_SEARCH_URL, str(mac_id))
		create_college = requests.post(ELASTIC_SEARCH_POST_URL, data=json.dumps(cc_details))
		if create_college.status_code == 201:
		    current_app.logger.debug("Added college details : %s " % (cc_details))
		    return jsonify({"status" : "success"})
		else:
		    current_app.logger.debug("Error in adding college details : %s " % (cc_details))
		    abort(int(college_data.status_code), "{'status' : 'Error in adding college details'}")
	    else:
		current_app.logger.debug("College details already exists : %s " % (cc_details))
...
** Dump Feedback to ELK
   REST API to get and add feedback to ELK stack

#+NAME: add_feedback_to_elk
#+BEGIN_SRC python
@api.route('/dump_feedback_to_elk', methods=['GET', 'POST'])
def dump_feedback_to_elk():
    if request.method == 'GET':
	return True
    if request.method == 'POST':
	#URL = "http://10.100.0.15:9200/college_cloud/details"
	data_json = json.dumps(request.get_json())
	data_dict = yaml.safe_load(data_json)
	if 'feedbacks' not in data_dict:
	    abort(400, 'feedbacks attribute is not passed')
	...
	key = data_dict['key']        
	current_app.logger.debug("Elastic Search URL : %s " % (ELASTIC_SEARCH_URL))

	if 'mac_id' in data_dict:
	    mac_id = str(data_dict['mac_id'])
	    r = requests.get(ELASTIC_SEARCH_URL+"/id="+mac_id)
		...
		index_name = str(mac_id) + "_"+ str(r.json()['_source']['college_name'])
		doc_name = "feedback"
...
** Dump Usage to ELK
   REST API to get and add usage to ELK stack

#+NAME: add_usage_to_elk
#+BEGIN_SRC python
@api.route('/dump_usage_to_elk', methods=['GET', 'POST'])
def dump_usage_to_elk():
	...
	if 'usages' not in data_dict or 'mac_id' not in data_dict:
	    abort(400, 'usages attribute or mac_id attribute is not passed')

	if 'key' not in data_dict:
	    abort(401, 'Unauthorized to perform this operation')

	usages = data_dict['usages']
	mac_id = data_dict['mac_id']
	key = data_dict['key']
	current_app.logger.debug("ELASTIC SEARCH URL : %s " % (ELASTIC_SEARCH_URL))
	try:
	    r = requests.get(ELASTIC_SEARCH_URL+"/id="+str(mac_id))
	    if r.json()['found'] == False:
		current_app.logger.debug("ELASTIC SEARCH URL : %s " % (ELASTIC_SEARCH_URL))
		return jsonify({"status" : "failed", "error" : "College cloud is not registered"})
	    else:
...
<header>
  ...
  <nav class="navbar navbar-default " style="background-color: white !important; margin-bottom: 0px !important; border : 0px;  !important">
	...
	<a class="navbar-brand main-logo-a" href="https://vlabs.ac.in"><img src="/static/img/logo.png"  style="width:7em" class="img-responsive main-logo" /></a>
      </div>
      <div class="collapse navbar-collapse menu-div" id="bs-example-navbar-collapse-1" style="border: 0px;margin-top:35px;">
	...
	<ul class="nav navbar-nav navbar-right menu-ul">
	  ...
	  <li class="page-scroll menu-li menu-li-active">
	    <a href="https://vlabs.ac.in/courses" class="menu-a menu-a-active">ALL LABS</a>
	  </li>
	  <li class="page-scroll menu-li ">
...
<div class="container-fluid footer-div" style="background: #404040; margin-top: 40px;  padding: 20px;">
  ...
  <div class="col-md-3 border-right-green-dotted" id="contact">
    ...
    <a href="http://118.102.191.38/labassessment/assessmentform.html" class="text-normal-gray-small nounderline" target="_blank" style="color:white;"> Lab Assessment Form</a><br>
  ...
  <div class="col-md-3 border-right-green-dotted">
    ...
    <a href="http://outreach.virtual-labs.ac.in/ncentres" class="text-normal-gray-small nounderline" style="color: white" target="_blank"> Nodal Centers</a><br>
    ...
    <a href="http://vlabs.ac.in:5959/announcements/index.html" class="text-normal-gray-small nounderline" style="color: white" target="_blank"> Announcements</a><br>
  ...
  <div class="col-md-2">
	...
	<ul>
	  <li style="display:inline"><a href="https://www.facebook.com/pages/Virtual-Labs-IIT-Delhi/301510159983871" target="_blank"><img src="/static/img/fb.png"></a></li>
	  <li style="display:inline "><a href="https://twitter.com/TheVirtualLabs" target="_blank"><img src="/static/img/twitter.png"></a></li>
	  <li style="display:inline"><a href="http://linkedin.com" target="_blank"><img src="/static/img/linkedin.png"></a></li> 
	  <li style="display:inline"><a href="https://www.youtube.com/watch?v=mVBYNXLaDdg" target="_blank"><img src="/static/img/youtube.png"></a></li>
	</ul>
      </ul>
    </div>
  </div>
...
from runtime.config.flask_app_config import GENERIC_FEEDBACK_VERSION,\
     GENERIC_LAB_FEEDBACK_VERSION, GENERIC_EXP_FEEDBACK_VERSION, ELASTIC_SEARCH_URL
