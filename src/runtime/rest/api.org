    ...
    if request.method == 'POST':
        try:
            ...
            cc_details_elastic_db_url = "%s/college_cloud/details/id=%s" \
              % (ELASTIC_DB_URL, mac_addr)
            print "==============================="
            print cc_details_elastic_db_url
            ...
            print "((((((((((((((((((((((((("
            print college_data
            print college_data.json()['found']
            if college_data.json()['found'] == False:
                print "%%%%%%%%%%%%%%%%%%%%%%%%"
                ELASTIC_SEARCH_POST_URL = "%s/college_cloud/details/id=%s" \
                  % (ELASTIC_DB_URL, str(mac_addr))
                print  ELASTIC_SEARCH_POST_URL
                current_app.logger.debug("ELASTIC_SEARCH_POST_URL : %s " \
                                                 % (ELASTIC_SEARCH_POST_URL))
                create_college = requests.post(ELASTIC_SEARCH_POST_URL,
                                           data=json.dumps(cc_details))
                print "************************"
                print create_college.status_code
		if create_college.status_code == 201:
		    current_app.logger.debug("Added college details : %s "
                                         % (cc_details))
		    return jsonify({"status" : "success"})
		else:
		    current_app.logger.debug("Error in adding college details : %s "
                                         % (cc_details))
