...
** Dump College Details into the Elastic Database
   ...
   - This API is used by the college cloud's post processing script
     which will dump the college cloud details which is json format into elastic search database.
   - Based on above college cloud details this API internally forms elastic
     search API as =/college-name_macid= to POST the data.
...
def dump_cc_details_to_elk():
  ...
  if request.method == 'POST':
	  ...
	  if 'college_details' not in data_dict:
	      print "college_details attribute is not passed"
	  ...
	  college_data = requests.get(cc_details_elastic_db_url)
          if college_data.json()['status'] == 404 or college_data.json()['found'] == False:
...
** Dump Feedback to ELK
   - This API is used by the college cloud's post processing script
     which will dump the college cloud feedback data which is in json
     format (list of feedbacks) into elastic search database.
   - Only If the college cloud is registered with elastic search
     database then only feedbacks will be dumped into database.
...
** Dump Usage to ELK
   ...
   - This API is used by the college cloud's post processing script
     which will dump the college cloud usage data which is in json
     format (list of usage) into elastic search database.
   - Only If the college cloud is registered with elastic search
     database then only usages will be dumped into database.
...
<h2 class="text-h2-lightblue-small" style="font-weight: bold;margin-top: 0px;"> Follow us</h2>-->
        <ul>
          <li style="display:inline"><a target="_blank" href="https://www.facebook.com/pages/Virtual-Labs-IIT-Delhi/301510159983871"><img src="images/fb.png"></a></li>
          <li style="display:inline "><a href="https://twitter.com/TheVirtualLabs"><img src="images/twitter.png"></a></li>
          <li style="display:inline"><a target="_blank" href="https://in.linkedin.com/in/virtual-labs-008ba9136"><img src="images/linkedin.png"></a></li>
          <li style="display:inline"><a target="_blank" href="https://www.youtube.com/watch?v=mVBYNXLaDdg"><img src="images/youtube.png"></a></li>
                           
        </ul>

        <div><img class="poweredby" src="images/logo-open-edx-powerdby.png"> </div>
