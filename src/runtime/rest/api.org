...
def dump_cc_details_to_elk():
    ...
    if request.method == 'POST':
        ...
        key = data_dict['key']
        current_app.logger.debug("Config data : %s " % (cc_details))
        mac_id = cc_details['mac_id']
        URL = "http://10.100.0.15:9200/college_cloud/details"
        current_app.logger.debug("URL : %s " % (URL))
        r = requests.get(URL+"/id="+str(mac_id))
        if r.json()['found'] == False:
            r = requests.post(URL+"/id="+mac_id, data=json.dumps(cc_details))
...
#+NAME: get_and_add_feedback_to_elk
#+BEGIN_SRC python
...
def dump_feedback_to_elk():
    ...
    if request.method == 'POST':
        ...
        if r.json()['found'] == False:
            current_app.logger.debug("URL : %s " % (URL))
            return jsonify({"status" : "failed", "error" : "College cloud details not registered"})
        else:
            current_app.logger.debug("Data : %s " % (r.json()))
            index_name = str(mac_id) + "_"+ str(r.json()['_source']['college_name'])
            doc_name = "feedback"
            current_app.logger.debug("index_name : %s " % (index_name))
