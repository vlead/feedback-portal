  ...
  def add_feedback():
      ...
      if request.mothod == 'POST':
          if not request.json or not 'lab_name' in request.json or not 'exp_name' in request.json or \
             not 'questions' in request.json or not 'key' in request.json:
              abort(400)
          else:
              ...
              try:
                  fb = SystemInterface()
                  user = fb.add_feedback(lab_name, exp_name, date,questions, key)
                  return jsonify(user)
...
class TestCreateUser(TestCase):
    ...
    def test_add_feedback(self):
        ...
        payload = {'lab_name': 'data structure',
                   ...
                   'responses: [{name:'Exp is working ', answers:['ans1', 'ans2'] }]}

        headers = {'content-type': 'application/json', 'key' : Config.key}
