...
** Dump College Details to Elastic Database
   REST API to dump the college cloud details into vlead's central
...
def dump_cc_details_to_elk():
	...
	try:
		...
		ELASTIC_SEARCH_POST_URL = "%s/college_cloud/details/id=%s" % (ELASTIC_DB_URL, str(mac_id))
                current_app.logger.debug("ELASTIC_SEARCH_POST_URL : %s " % (ELASTIC_SEARCH_POST_URL))
...
*** TestDumpCollegeDetailsToElk
 ...
 class TestDumpCollegeDetailsToElk(TestCase):
	 ...
	 response = self.client.post("/dump_cc_details_to_elastic_db", data=json.dumps(payload),
				  headers=headers)
...
** Dump Feedback to ELK
   REST API to dump the feedbacks into the elastic search database
...
*** TestDumpFeedbackDetailsToElk
 ...
 class TestFeedbackDumpToElk(TestCase):
     ...
     def test_feedback_details_to_elk(self):
	 print "test_dump_college_details_to_elk"
	 payload = {
	     'key' : KEY,
	     'feedbacks': [
...
** Dump Usage to ELK
   REST API to dump the usage data into elastic search database.
