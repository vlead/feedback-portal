...
def dump_cc_details_to_elk():
	...
	try:
	    ...
	    ELASTIC_SEARCH_GET_URL = "%s/id=%s" %(ELASTIC_DB_URL, str(mac_id))
		...
		ELASTIC_SEARCH_POST_URL = "%s/id=%s" % (ELASTIC_DB_URL, str(mac_id))
...
def dump_feedback_to_elk():
	...
	if 'mac_id' in data_dict:
	    mac_id = str(data_dict['mac_id'])
            cc_details_elastic_db_url = "%s/college_cloud/details/id=%s" % (ELASTIC_DB_URL, mac_id)
