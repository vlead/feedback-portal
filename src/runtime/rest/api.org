...
def dump_cc_details_to_elk():
	...
	try:
		...
		ELASTIC_SEARCH_POST_URL = "%s/college_cloud/details/id=%s" % (ELASTIC_DB_URL, str(mac_addr))
...
def dump_feedback_to_elk():
	...
	if 'mac_addr' in data_dict:
		...
		index_name = str(mac_addr) + "_"+ str(r.json()['_source']['college_name'])
		doc_name = "feedback"
...
def dump_usage_to_elk():
	...
	if 'usages' not in data_dict or 'mac_addr' not in data_dict:
	    abort(400, 'usages attribute or mac_addr attribute is not passed')
