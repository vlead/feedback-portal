#+TITLE: Feedback Portal 3.0 Data Model
#+AUTHOR: VLEAD
#+DATE: [2016-08-29 Mon]
...
1) Gateway IP, User id, Lab id, Discipline id will be provided by the OpenedX.
...
#+BEGIN_SRC spec :tangle spec.txt
  [feedback, response]   : feedback-set !  >  response-set +
  [response, question]   : response-set    >  question-set !
  [response, answer]     : response-set    >  answer-set + 
  [workshop, feedback]   : feedback-set    >  workshop-set !
...
*** Actor credentials
#+BEGIN_SRC spec :tangle spec.txt
    s.key == key where key:key need to be passed by the client 
#+END_SRC   
...
#+BEGIN_SRC spec :tangle spec.txt
    ...
    A feedback with the given responses is created and added to feedback-set
#+END_SRC   
